...
class arcWidget(QtGui.QWidget):
    def __init__(self, arcdisplay, hmin=150, wmin=450, name=None,x1=0, w1=0, y1=None, y2=None,  parent=None):
        ...
        self.funcComboBox.setCurrentIndex(self.arcdisplay.ws.func_options.index(self.arcdisplay.ws.function))
        ...
        self.methodComboBox.setCurrentIndex(ai.autoidentify_options.index(self.arcdisplay.method))
        ...
        self.connect(self.arcdisplay, QtCore.SIGNAL('keyPressEvent'), self.keyPressEvent)
        self.connect(self.arcdisplay, QtCore.SIGNAL('updatex(float)'), self.updatexlabel)
        self.connect(self.funcComboBox, QtCore.SIGNAL('activated(QString)'), self.updatefunction)
        self.connect(self.methodComboBox, QtCore.SIGNAL('activated(QString)'), self.updatemethod)
...
class errWidget(QtGui.QWidget):
    def __init__(self, arcdisplay, hmin=150, wmin=450, name=None, parent=None):
        ...
        self.sigmaValueEdit = QtGui.QLineEdit("%2.1f"% self.arcdisplay.ws.thresh)
    ...
    def fitUpdate(self):
        if len(self.arcdisplay.xp)<=1:  return
        try:
            xp=np.array(self.arcdisplay.xp)
            wp=np.array(self.arcdisplay.wp)
            w=self.arcdisplay.ws.value(xp)
...
class ArcDisplay(QtGui.QWidget):
 ...
 """
    ...
    def findxcorfit(self):
        ...
        """
        self.ws=st.fitxcor(self.xarr, self.farr, self.swarr, self.sfarr, self.ws, interptype='interp') 
        self.plotArt()
        self.redraw_canvas()
    def findfit(self):
