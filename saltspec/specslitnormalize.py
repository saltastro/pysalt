...
def specslitnormalize(images, outimages, outpref, response=None, response_output=None, order=2, conv=1e-2, niter=20,
       ...
       for img, ofile in zip(infiles, outfiles):
           ...
           for i in range(startext, len(hdu)):
               if hdu[i].name=='SCI':
                   ...
                   if response is None:
                       response = create_response(data, spatial_axis=1, order=order, conv=conv, niter=niter)
                       if response_output: write_response(response, clobber=clobber)
                   else:
                       #add a check that the response is the same shape as the data
                       if len(response)!=data.shape[0]:
                           raise SALTSpecError('Length of response function does not equal size of image array')
...
def write_response(response, response_output, clobber):
    if os.path.isfile(response_output) and clobber: os.remove(response_output)
    savetxt(response_output, response)

def create_response(data, spatial_axis=1, order=2, conv=1e-2, niter=20):
    """Create the slit response function.   This fits a low order polynomial across the spatial
       dimension of the image and returns the result
...
t = iraf.IrafTaskFactory(taskname="specslitnormalize",value=parfile,function=specslitnormalize, pkgname='saltspec')
