c------------------------------------------------------------------------------
      subroutine mrqfit2d (x, y, sig, mdata, ndata, a, afit, npar, 
     $     covar, chisq, funk)
c------------------------------------------------------------------------------
...
c    CHISQ chi squared for the fit; real
      ...
      real    x(mdata, ndata), y(ndata), sig(ndata)
      real    a(npar), covar(npar,npar)
      real    ocovar(MMAX,MMAX), oa(MMAX), da(MMAX), oda(MMAX)
      real    alamda, chisq, ochisq, funk, swap
      logical afit(npar), trouble, conver
      integer mdata, ndata, npar, i, j, k, nfit
      ...
      alamda = 0.001
      call mrqcof  (x, y, sig, mdata, ndata, a, afit, npar, nfit,
...
c------------------------------------------------------------------------------
      subroutine mrqcof (x, y, sig, mdata, ndata, 
     $                    a, afit, npar, nfit,
...
c    CHISQ chi squared for the fit; real
      ...
      real     x(mdata, ndata), y(ndata), sig(ndata), chisq
      real     a(npar), alpha(nalp,nalp), beta(nalp)
      real     dyda(MMAX), dy, wgt, tmp, yfit
      integer  nalp, mdata, ndata, nfit, npar
      integer  i, j, k, l, m
      logical  afit(npar)
      external funk
      ...
      chisq = 0.0
      do i=1,ndata
         call funk (x(1,i), a, npar, yfit, dyda)
         wgt = 1.0 / (sig(i) * sig(i))
         dy = y(i) - yfit
