...
C     Updated to read which etalon is in use if only one and to read from header.
      ...
      write(*,'(a)') etstate(6:7)
      if (etstate(6:7) .eq. 'S2') then !Et1
         etalonz='ET1Z'
         etalonl='ET1WAVE0'
      else if (etstate(6:7) .eq. 'S3') then !Et2
         etalonz='ET2Z'
         etalonl='ET2WAVE0'
      else
      ...
      ier=0
      call ftgkye (luimg, etalonl, wave, comment, ier)
      do while (ier .ne. 0)
...
c  display and query limits
      ...
      if (plot) then
c         call ringpro (ring, isize(1), isize(2), .false., x, y, dy)
	   xmin = x(1)
	   xmax = x(1)
         ymin = y(1)
         ymax = y(1)
...
c	call ringcen (ring,isize(1),isize(2),fixed,rave,rerr,rxc,ryc)
	if (rave .lt. 0.0) then
	   ier = nint(rave)
